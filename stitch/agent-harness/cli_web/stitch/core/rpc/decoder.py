...
def extract_result(chunks: list[str], rpc_id: str) -> Any:
    ...
    Raises:
        ...
        RPCError: If the expected rpc_id is not found in any chunk
    """
    for chunk in chunks:
        try:
        ...
        for entry in outer:
            ...
            if entry[0] == "er":
                ...
                if err_code in (7, 9):
                    raise AuthError(f"Auth error (code {err_code}) — run: cli-web-stitch auth login")
    ...
    raise RPCError(f"RPC result for '{rpc_id}' not found in response")
